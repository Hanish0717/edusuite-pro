...
import { Sparkles, Send, Bot, User, Maximize2 } from "lucide-react";

interface AiAssistantCardProps {
  onOpenFullAssistant: () => void;
}

export function StudentAiAssistantCard({ onOpenFullAssistant }: AiAssistantCardProps) {
  ...
  const handleSend = (textToSend?: string) => {
    ...
    setTimeout(() => {
      ...
      } else if (queryLower.includes("next class") || queryLower.includes("class")) {
      ...
      } else if (queryLower.includes("dbms") || queryLower.includes("normalization")) {
        aiText = "DBMS Normalization is the process of organizing relational tables to reduce redundancy. Key Normal Forms: 1NF (atomic values), 2NF (no partial dependency), 3NF (no transitive dependency), BCNF (every determinant is a candidate key).";
      }
