...
export const Route = createFileRoute("/staff/finance-dean/invoice-management")({
  head: () => ({
    meta: [{ title: "Invoice Management — Finance Dean" }],
  }),
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Vendor invoice processing, 3-way matching with PO & Goods Receipt.</p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Invoices Processed" value="42 Invoices" icon={Wallet} tone="purple" />
        <KpiCard label="3-Way Matched" value="100%" icon={CreditCard} tone="success" />
        <KpiCard label="Cleared Amount" value="₹1.85 Cr" icon={TrendingUp} tone="info" />
      ...
      {/* MAIN DATA TABLE */}
      <Panel title="Invoice Management Master Ledger" description="Official institutional financial ledgers, audit vouchers, and budget allocations.">
          ...
          <div className="overflow-x-auto border border-border rounded-xl">
            <table className="w-full text-left text-xs">
              <thead className="border-b border-border bg-muted/40 font-semibold uppercase tracking-wider text-muted-foreground">
                <tr>
                  <th className="p-3">Invoice No</th><th className="p-3">Vendor Name</th><th className="p-3">PO Number</th><th className="p-3">Invoice Date</th><th className="p-3">Invoice Amount</th><th className="p-3">Payment Status</th><th className="p-3">Status</th>
                </tr>
              </thead>
