...
import { getImaDashboardData } from "@/lib/deansService";

export const Route = createFileRoute("/staff/ima/asset-audit")({
  head: () => ({
    meta: [{ title: "Capital Asset Audit — EduSuite Pro" }],
  }),
  component: SubPageComponent,
});

function SubPageComponent() {
  const data = useMemo(() => getImaDashboardData(), []);
  ...
  const rawItems = useMemo(() => {
    return [ { tag: 'AST-2026-001', desc: 'NVIDIA H100 GPU Supercomputing Server', dept: 'CSE AI Lab', val: '₹45 Lakhs', status: 'Verified' }, { tag: 'AST-2026-002', desc: 'Solar Inverter Unit 100kW', dept: 'Green Energy Cell', val: '₹12 Lakhs', status: 'Verified' } ];
  }, [data]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Capital Asset Audit</h1>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Asset Pass Rate" value="99.2%" icon={Building2} tone="info" />
        <KpiCard label="Total Assets Tagged" value="14,250" icon={Users} tone="success" />
        <KpiCard label="Valuation" value="₹48.5 Cr" icon={ShieldCheck} tone="purple" />
        <KpiCard label="Status" value="Verified" icon={CheckCircle2} tone="warning" />
      </div>

      {/* FILTERABLE MAIN TABLE */}
      <Panel title="Capital Asset Audit Ledger" description="Official domain records & ERP status.">
          ...
          <div className="overflow-x-auto border border-border rounded-xl">
            <table className="w-full text-left text-xs">
              <thead className="border-b border-border bg-muted/40 font-semibold uppercase tracking-wider text-muted-foreground">
                <tr>
                  <th className="p-3">Asset Tag</th><th className="p-3">Asset Description</th><th className="p-3">Department</th><th className="p-3">Valuation</th><th className="p-3">Audit Status</th>
                </tr>
              </thead>
