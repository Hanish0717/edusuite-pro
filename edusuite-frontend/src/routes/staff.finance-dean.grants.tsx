...
export const Route = createFileRoute("/staff/finance-dean/grants")({
  head: () => ({
    meta: [{ title: "Grant Disbursements & Audit — EduSuite Pro" }],
  }),
...
function SubPageComponent() {
  ...
  const rawItems = useMemo(() => {
    return [ { ref: 'GRT-DST-2026', project: 'DST SERB Edge AI Research', sanctioned: '₹85.0 Lakhs', disbursed: '₹85.0 Lakhs', status: 'Disbursed' } ];
  }, [data]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Grant Disbursements & Audit</h1>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Disbursed Grants" value="₹4.85 Cr" icon={Building2} tone="info" />
        <KpiCard label="Audit Clearance" value="100% Pass" icon={Users} tone="success" />
        <KpiCard label="Utilization SLA" value="On Track" icon={ShieldCheck} tone="purple" />
        <KpiCard label="Status" value="Verified" icon={CheckCircle2} tone="warning" />
      </div>

      {/* FILTERABLE MAIN TABLE */}
      <Panel title="Grant Disbursements & Audit Ledger" description="Official domain records & ERP status.">
          ...
          <div className="overflow-x-auto border border-border rounded-xl">
            <table className="w-full text-left text-xs">
              <thead className="border-b border-border bg-muted/40 font-semibold uppercase tracking-wider text-muted-foreground">
                <tr>
                  <th className="p-3">Grant Ref</th><th className="p-3">Project Recipient</th><th className="p-3">Sanctioned Amount</th><th className="p-3">Disbursed Amount</th><th className="p-3">Status</th>
                </tr>
              </thead>
