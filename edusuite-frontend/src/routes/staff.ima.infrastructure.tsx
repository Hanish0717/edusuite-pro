...
export const Route = createFileRoute("/staff/ima/infrastructure")({
  head: () => ({
    meta: [{ title: "Campus Infrastructure Assets — EduSuite Pro" }],
  }),
...
function SubPageComponent() {
  ...
  const rawItems = useMemo(() => {
    return [ { name: 'Advanced Supercomputing Building', category: 'Academic Lab', area: '25,000 sq.ft', util: '92%', status: 'Active' }, { name: 'Central Engineering Auditorium', category: 'Auditorium', area: '18,000 sq.ft', util: '88%', status: 'Active' } ];
  }, [data]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Campus Infrastructure Assets</h1>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Total Buildings" value="14 Blocks" icon={Building2} tone="info" />
        <KpiCard label="Lab Floor Area" value="120,000 sq.ft" icon={Users} tone="success" />
        <KpiCard label="Occupancy Rate" value="94.5%" icon={ShieldCheck} tone="purple" />
        <KpiCard label="Status" value="Active" icon={CheckCircle2} tone="warning" />
      </div>

      {/* FILTERABLE MAIN TABLE */}
      <Panel title="Campus Infrastructure Assets Ledger" description="Official domain records & ERP status.">
          ...
          <div className="overflow-x-auto border border-border rounded-xl">
            <table className="w-full text-left text-xs">
              <thead className="border-b border-border bg-muted/40 font-semibold uppercase tracking-wider text-muted-foreground">
                <tr>
                  <th className="p-3">Asset Name</th><th className="p-3">Category</th><th className="p-3">Floor Area</th><th className="p-3">Space Utilization</th><th className="p-3">Status</th>
                </tr>
              </thead>
