...
export const Route = createFileRoute("/staff/examination-dean/paper-distribution")({
  head: () => ({
    meta: [{ title: "Paper Distribution — Examination Dean" }],
  }),
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Paper Distribution</h1>
          <p className="text-sm text-muted-foreground">Exam hall question paper print dispatch & digital distribution audit.</p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Dispatched Halls" value="36 Halls" icon={Users} tone="purple" />
        <KpiCard label="Print Passcode" value="Generated" icon={Calendar} tone="success" />
        <KpiCard label="Dispatch SLA" value="T-30m" icon={Ticket} tone="info" />
        <KpiCard label="Status" value="Dispatched" icon={Award} tone="warning" />
      </div>

      

      {/* MAIN DATA TABLE */}
      <Panel title="Paper Distribution Master Ledger" description="Official Controller of Examinations records, schedules, marks entries, and hall allocations.">
          ...
          <div className="overflow-x-auto border border-border rounded-xl">
            <table className="w-full text-left text-xs">
              <thead className="border-b border-border bg-muted/40 font-semibold uppercase tracking-wider text-muted-foreground">
                <tr>
                  <th className="p-3">Exam Date</th><th className="p-3">Session</th><th className="p-3">Hall Number</th><th className="p-3">Subject Code</th><th className="p-3">Print Dispatch Time</th><th className="p-3">Hall Invigilator</th><th className="p-3">Status</th>
                </tr>
              </thead>
