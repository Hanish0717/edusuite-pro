...
export const Route = createFileRoute("/staff/examination-dean/revaluation-requests")({
  head: () => ({
    meta: [{ title: "Revaluation Requests — Examination Dean" }],
  }),
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Revaluation Requests</h1>
          <p className="text-sm text-muted-foreground">Student revaluation & script recounting application requests.</p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Revaluation Reqs" value="14 Requests" icon={Users} tone="purple" />
        <KpiCard label="Fee Paid" value="₹14,000" icon={Calendar} tone="success" />
        <KpiCard label="Camp Allocated" value="100%" icon={Ticket} tone="info" />
        <KpiCard label="Status" value="Active" icon={Award} tone="warning" />
      </div>

      
      <Panel title="Revaluation Requests Statistics Chart" description="Quantitative examination ledgers across academic departments.">
        <GroupedBarChart
          data={[
      ...
      {/* MAIN DATA TABLE */}
      <Panel title="Revaluation Requests Master Ledger" description="Official Controller of Examinations records, schedules, marks entries, and hall allocations.">
          ...
          <div className="overflow-x-auto border border-border rounded-xl">
            <table className="w-full text-left text-xs">
              <thead className="border-b border-border bg-muted/40 font-semibold uppercase tracking-wider text-muted-foreground">
                <tr>
                  <th className="p-3">Application ID</th><th className="p-3">Student Name</th><th className="p-3">Roll Number</th><th className="p-3">Subject Code</th><th className="p-3">Subject Title</th><th className="p-3">Original Marks</th><th className="p-3">Fee Paid</th><th className="p-3">Status</th>
                </tr>
              </thead>
