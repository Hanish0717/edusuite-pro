...
export const Route = createFileRoute("/staff/examination-dean/timetable-generation")({
  head: () => ({
    meta: [{ title: "Timetable Generation — Examination Dean" }],
  }),
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Timetable Generation</h1>
          <p className="text-sm text-muted-foreground">Automated exam timetable scheduling algorithm, gap checks, and room conflict resolution.</p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Conflict Free" value="100% Passed" icon={Users} tone="purple" />
        <KpiCard label="Gaps Verified" value="1 Day Between Exams" icon={Calendar} tone="success" />
        <KpiCard label="Algorithm Status" value="Optimized" icon={Ticket} tone="info" />
        <KpiCard label="Status" value="Approved" icon={Award} tone="warning" />
      </div>

      

      {/* MAIN DATA TABLE */}
      <Panel title="Timetable Generation Master Ledger" description="Official Controller of Examinations records, schedules, marks entries, and hall allocations.">
          ...
          <div className="overflow-x-auto border border-border rounded-xl">
            <table className="w-full text-left text-xs">
              <thead className="border-b border-border bg-muted/40 font-semibold uppercase tracking-wider text-muted-foreground">
                <tr>
                  <th className="p-3">Schedule Ref</th><th className="p-3">Department Scope</th><th className="p-3">Total Subjects</th><th className="p-3">Conflict Checks</th><th className="p-3">Generated Date</th><th className="p-3">Status</th>
                </tr>
              </thead>
