...
export const Route = createFileRoute("/staff/examination-dean/recounting")({
  head: () => ({
    meta: [{ title: "Recounting — Examination Dean" }],
  }),
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Recounting</h1>
          <p className="text-sm text-muted-foreground">Totaling error verification and script totaling re-check ledger.</p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Recounting Reqs" value="4 Requests" icon={Users} tone="purple" />
        <KpiCard label="Totaling Error" value="1 Corrected" icon={Calendar} tone="success" />
        <KpiCard label="Verification SLA" value="Passed" icon={Ticket} tone="info" />
      ...
      {/* MAIN DATA TABLE */}
      <Panel title="Recounting Master Ledger" description="Official Controller of Examinations records, schedules, marks entries, and hall allocations.">
          ...
          <div className="overflow-x-auto border border-border rounded-xl">
            <table className="w-full text-left text-xs">
              <thead className="border-b border-border bg-muted/40 font-semibold uppercase tracking-wider text-muted-foreground">
                <tr>
                  <th className="p-3">Recount ID</th><th className="p-3">Student Name</th><th className="p-3">Subject Code</th><th className="p-3">Original Total</th><th className="p-3">Recounted Total</th><th className="p-3">Status</th>
                </tr>
              </thead>
