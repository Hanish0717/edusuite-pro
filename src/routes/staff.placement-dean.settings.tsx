...
export const Route = createFileRoute("/staff/placement-dean/settings")({
  head: () => ({
    meta: [{ title: "Placement Dean Portal Settings — EduSuite Pro" }],
  }),
...
function SubPageComponent() {
  ...
  const rawItems = useMemo(() => {
    return [ { name: 'Default Placement Eligibility CGPA', val: '7.00 CGPA & 0 Active Backlogs', date: '2026-01-10', status: 'Active' } ];
  }, [data]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Placement Dean Portal Settings</h1>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Placement Dean" value="Prof. Placement Dean" icon={Building2} tone="info" />
        <KpiCard label="Eligibility Criteria" value="7.0 CGPA & No Backlogs" icon={Users} tone="success" />
        <KpiCard label="Portal Version" value="v2026.2" icon={ShieldCheck} tone="purple" />
        <KpiCard label="Status" value="Active" icon={CheckCircle2} tone="warning" />
      </div>

      {/* FILTERABLE MAIN TABLE */}
      <Panel title="Placement Dean Portal Settings Ledger" description="Official domain records & ERP status.">
          ...
          <div className="overflow-x-auto border border-border rounded-xl">
            <table className="w-full text-left text-xs">
              <thead className="border-b border-border bg-muted/40 font-semibold uppercase tracking-wider text-muted-foreground">
                <tr>
                  <th className="p-3">Setting Parameter</th><th className="p-3">Configured Value</th><th className="p-3">Last Modified</th><th className="p-3">Status</th>
                </tr>
              </thead>
