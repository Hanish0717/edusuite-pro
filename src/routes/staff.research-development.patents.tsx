...
export const Route = createFileRoute("/staff/research-development/patents")({
  head: () => ({
    meta: [{ title: "Patents & IPR Filings — EduSuite Pro" }],
  }),
...
function SubPageComponent() {
  ...
  const rawItems = useMemo(() => {
    return data.patents.map(pt => ({ title: pt.title, no: pt.patentNo, date: pt.filingDate, status: pt["status"] }));
  }, [data]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Patents & IPR Filings</h1>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Patents Filed" value="18 Patents" icon={Building2} tone="info" />
        <KpiCard label="Published" value="12 Patents" icon={Users} tone="success" />
        <KpiCard label="Granted" value="4 Patents" icon={ShieldCheck} tone="purple" />
        <KpiCard label="Status" value="Active" icon={CheckCircle2} tone="warning" />
      </div>

      {/* FILTERABLE MAIN TABLE */}
      <Panel title="Patents & IPR Filings Ledger" description="Official domain records & ERP status.">
          ...
          <div className="overflow-x-auto border border-border rounded-xl">
            <table className="w-full text-left text-xs">
              <thead className="border-b border-border bg-muted/40 font-semibold uppercase tracking-wider text-muted-foreground">
                <tr>
                  <th className="p-3">Patent Title</th><th className="p-3">Patent Application No</th><th className="p-3">Filing Date</th><th className="p-3">Status</th>
                </tr>
              </thead>
