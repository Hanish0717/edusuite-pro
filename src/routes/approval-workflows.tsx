...
function ApprovalWorkflowsPage() {
  ...
  const filteredWorkflows = useMemo(() => {
    return workflows.filter((wf) => {
      ...
      const matchesPriority = priorityFilter === "All Priorities" || wf.priority === priorityFilter;
  ...
  const summaryMetrics = useMemo(() => {
    const totalPending = workflows.filter((w) => w.status === "Pending" || w.status === "In Review" || w.status === "Escalated").length;
    const approvedToday = 8;
    const rejectedToday = 2;
    const criticalRequests = workflows.filter((w) => w.priority === "Critical").length;
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs bg-muted/30 p-3 rounded-xl border border-border/60">
                      <div>
                        ...
                        <span className="font-bold text-foreground font-sans">{wf.currentApprover}</span>
                      </div>
                      ...
                      <div>
                        ...
                        <span className="font-bold text-emerald-600 font-mono flex items-center gap-1">
                          <Clock className="size-3 text-emerald-500" /> {wf.slaRemaining}
                        </span>
                      </div>
                      ...
                      <div>
                        ...
                        <Badge
                          className={
                            wf.status === "Approved" || wf.status === "Completed"
                    ...
                    <div className="p-4 rounded-xl bg-card border border-border/80 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                      ...
                      <div className="flex items-center gap-1.5 flex-wrap shrink-0">
                        ...
                        <Button
                          ...
                          className="h-8 text-xs gap-1"
                        >
                          <Paperclip className="size-3.5" /> Documents ({wf.documents?.length || 0})
                        </Button>
                        ...
                        <Button
                          ...
                          disabled={wf.priority === "Critical" || isCompleted}
            ...
            <div className="space-y-4 pt-1 text-xs">
              ...
              <div className="grid grid-cols-2 gap-2 font-mono">
                ...
                <div className="p-2.5 rounded-lg bg-card border border-border/60">
                  ...
                  <span className="text-emerald-600 font-bold text-xs">{selectedWorkflow.slaRemaining}</span>
                </div>
              </div>
        ...
        <DialogContent className="max-w-md">
          ...
          {selectedWorkflow && (
            <div className="space-y-3 pt-1 text-xs">
              <div className="space-y-2">
                {selectedWorkflow.documents?.map((doc, idx) => (
                  <div
                    key={idx}
        ...
        <DialogContent className="max-w-lg">
          ...
          {selectedWorkflow && (
            <div className="space-y-3 pt-1 text-xs max-h-[50vh] overflow-y-auto pr-1">
              {selectedWorkflow.history?.map((h, idx) => (
