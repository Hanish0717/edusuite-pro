...
export const Route = createFileRoute("/staff/iqac/feedback")({
  head: () => ({
    meta: [{ title: "Stakeholder Feedback Analysis — EduSuite Pro" }],
  }),
...
function SubPageComponent() {
  ...
  const rawItems = useMemo(() => {
    return [ { group: 'Students (UG)', count: '4,850', rate: '94.2%', target: '90.0%', status: 'Exceeds Target' }, { group: 'Alumni (2025)', count: '640', rate: '96.5%', target: '90.0%', status: 'Exceeds Target' }, { group: 'Employers (Tier 1)', count: '78', rate: '92.0%', target: '88.0%', status: 'Exceeds Target' } ];
  }, [data]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Stakeholder Feedback Analysis</h1>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Student Feedback" value="94.2%" icon={Building2} tone="info" />
        <KpiCard label="Alumni Rating" value="4.8 / 5" icon={Users} tone="success" />
        <KpiCard label="Employer Rating" value="4.7 / 5" icon={ShieldCheck} tone="purple" />
        <KpiCard label="Status" value="Verified" icon={CheckCircle2} tone="warning" />
      </div>

      {/* FILTERABLE MAIN TABLE */}
      <Panel title="Stakeholder Feedback Analysis Ledger" description="Official domain records & ERP status.">
          ...
          <div className="overflow-x-auto border border-border rounded-xl">
            <table className="w-full text-left text-xs">
              <thead className="border-b border-border bg-muted/40 font-semibold uppercase tracking-wider text-muted-foreground">
                <tr>
                  <th className="p-3">Stakeholder Group</th><th className="p-3">Total Respondents</th><th className="p-3">Satisfaction Rate</th><th className="p-3">Target Benchmark</th><th className="p-3">Review Status</th>
                </tr>
              </thead>
