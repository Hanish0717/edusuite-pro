...
export const Route = createFileRoute("/staff/examination-dean/hall-tickets")({
  head: () => ({
    meta: [{ title: "Hall Ticket Generation & Dispatch — EduSuite Pro" }],
  }),
...
function SubPageComponent() {
  ...
  const rawItems = useMemo(() => {
    return [ { batch: 'B.Tech CSE Sem 5', eligible: '480', generated: '468', blocked: '12 Shortage', status: 'Dispatched' }, { batch: 'B.Tech ECE Sem 5', eligible: '360', generated: '352', blocked: '8 Shortage', status: 'Dispatched' } ];
  }, [data]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Hall Ticket Generation & Dispatch</h1>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Hall Tickets Dispatched" value="4,850 Issued" icon={Building2} tone="info" />
        <KpiCard label="Shortage Blocked" value="142 Students" icon={Users} tone="success" />
        <KpiCard label="Generation SLA" value="100%" icon={ShieldCheck} tone="purple" />
        <KpiCard label="Status" value="Active" icon={CheckCircle2} tone="warning" />
      </div>

      {/* FILTERABLE MAIN TABLE */}
      <Panel title="Hall Ticket Generation & Dispatch Ledger" description="Official domain records & ERP status.">
          ...
          <div className="overflow-x-auto border border-border rounded-xl">
            <table className="w-full text-left text-xs">
              <thead className="border-b border-border bg-muted/40 font-semibold uppercase tracking-wider text-muted-foreground">
                <tr>
                  <th className="p-3">Batch / Sem</th><th className="p-3">Total Eligible</th><th className="p-3">Tickets Generated</th><th className="p-3">Blocked (Shortage)</th><th className="p-3">Status</th>
                </tr>
              </thead>
