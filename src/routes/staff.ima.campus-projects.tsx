...
export const Route = createFileRoute("/staff/ima/campus-projects")({
  head: () => ({
    meta: [{ title: "Campus Capital Projects — EduSuite Pro" }],
  }),
...
function SubPageComponent() {
  ...
  const rawItems = useMemo(() => {
    return data.campusProjects.map(p => ({ name: p.name, budget: p.budget, progress: p.progress + '%', date: p.targetDate, status: p.progress > 80 ? 'Near Completion' : 'In Progress' }));
  }, [data]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Campus Capital Projects</h1>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Active Projects" value="8 Projects" icon={Building2} tone="info" />
        <KpiCard label="Total Capital" value="₹12.4 Cr" icon={Users} tone="success" />
        <KpiCard label="Avg Progress" value="72.5%" icon={ShieldCheck} tone="purple" />
        <KpiCard label="Status" value="On Track" icon={CheckCircle2} tone="warning" />
      </div>

      {/* FILTERABLE MAIN TABLE */}
      <Panel title="Campus Capital Projects Ledger" description="Official domain records & ERP status.">
          ...
          <div className="overflow-x-auto border border-border rounded-xl">
            <table className="w-full text-left text-xs">
              <thead className="border-b border-border bg-muted/40 font-semibold uppercase tracking-wider text-muted-foreground">
                <tr>
                  <th className="p-3">Project Name</th><th className="p-3">Allocated Budget</th><th className="p-3">Progress %</th><th className="p-3">Target Date</th><th className="p-3">Status</th>
                </tr>
              </thead>
