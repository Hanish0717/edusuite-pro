...
export const Route = createFileRoute("/staff/examination-dean/revaluation")({
  head: () => ({
    meta: [{ title: "Revaluation & Recounting Requests — EduSuite Pro" }],
  }),
...
function SubPageComponent() {
  ...
  const rawItems = useMemo(() => {
    return data.revaluations.map(r => ({ id: r.id, student: r.student, subject: r.subject, grade: r.currentGrade, status: r.status }));
  }, [data]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Revaluation & Recounting Requests</h1>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Revaluation Requests" value="14 Requests" icon={Building2} tone="info" />
        <KpiCard label="Under Review" value="4 Cases" icon={Users} tone="success" />
        <KpiCard label="Grades Updated" value="10 Cases" icon={ShieldCheck} tone="purple" />
        <KpiCard label="Status" value="Active" icon={CheckCircle2} tone="warning" />
      </div>

      {/* FILTERABLE MAIN TABLE */}
      <Panel title="Revaluation & Recounting Requests Ledger" description="Official domain records & ERP status.">
          ...
          <div className="overflow-x-auto border border-border rounded-xl">
            <table className="w-full text-left text-xs">
              <thead className="border-b border-border bg-muted/40 font-semibold uppercase tracking-wider text-muted-foreground">
                <tr>
                  <th className="p-3">Request Ref ID</th><th className="p-3">Student Name</th><th className="p-3">Subject</th><th className="p-3">Current Grade</th><th className="p-3">Status</th>
                </tr>
              </thead>
