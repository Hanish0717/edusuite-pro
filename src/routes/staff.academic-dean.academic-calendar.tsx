...
export const Route = createFileRoute("/staff/academic-dean/academic-calendar")({
  head: () => ({
    meta: [{ title: "Academic Calendar — Academic Dean" }],
  }),
...
function SubPageComponent() {
  ...
  const headers = ["Academic Event Title","Target Batches","Start Date","End Date","Instructional Scope","Status"];
  ...
  const handleExportPDF = () => {
    toast.success("Academic Calendar PDF Report generated & ready for download.");
    window.print();
  };
  ...
  const handleExportExcel = () => {
    ...
    a.href = url;
    a.download = "academic-calendar-ledger.csv";
    a.click();
  ...
  const handleAddSubmit = (e: React.FormEvent) => {
    ...
    setIsAddOpen(false);
    setFormData({});
    toast.success("New Academic Calendar record created successfully!");
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Institutional academic calendar, instructional days, mid-terms, and vacation schedules.</p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Working Days" value="90 Days" icon={Building2} tone="purple" onClick={() => { setFilter("all"); toast.info("Filter reset to all records"); }} className="cursor-pointer hover:border-primary/50 transition-all" />
        <KpiCard label="Academic Term" value="Autumn 2026" icon={Users} tone="success" onClick={() => { setFilter("active"); toast.info("Filtering active records"); }} className="cursor-pointer hover:border-primary/50 transition-all" />
        <KpiCard label="Exam Weeks" value="3 Weeks" icon={BookOpen} tone="info" onClick={() => { setFilter("completed"); toast.info("Filtering completed records"); }} className="cursor-pointer hover:border-primary/50 transition-all" />
        <KpiCard label="Status" value="Published" icon={Award} tone="warning" onClick={() => { setFilter("pending"); toast.info("Filtering pending records"); }} className="cursor-pointer hover:border-primary/50 transition-all" />
      </div>

      

      {/* MAIN DATA TABLE */}
      <Panel title="Academic Calendar Master Ledger" description="Official executive records, interactive search, filters, pagination, and actions.">
        <div className="space-y-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="relative flex-1 max-w-sm">
              <Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" />
              <Input
                placeholder="Search academic calendar records..."
                value={search}
                onChange={(e) => {
      ...
      <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Add New Academic Calendar Record</DialogTitle>
      ...
      <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Edit Academic Calendar Record</DialogTitle>
      ...
      <Dialog open={isViewOpen} onOpenChange={setIsViewOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Academic Calendar Record Details</DialogTitle>
