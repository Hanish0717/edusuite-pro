...
export const Route = createFileRoute("/staff/iqac/quality-audits")({
  head: () => ({
    meta: [{ title: "Internal Quality Audits — EduSuite Pro" }],
  }),
...
function SubPageComponent() {
  ...
  const rawItems = useMemo(() => {
    return data.qualityAudits.map(q => ({ id: q.id, dept: q.dept, type: q.type, score: q.score, auditor: 'Dr. External Auditor', date: q.date }));
  }, [data]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Internal Quality Audits</h1>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Total Audits" value="24 Completed" icon={Building2} tone="info" />
        <KpiCard label="Avg Audit Score" value="4.78 / 5" icon={Users} tone="success" />
        <KpiCard label="Audit Compliance" value="98.5%" icon={ShieldCheck} tone="purple" />
        <KpiCard label="Status" value="Verified" icon={CheckCircle2} tone="warning" />
      </div>

      {/* FILTERABLE MAIN TABLE */}
      <Panel title="Internal Quality Audits Ledger" description="Official domain records & ERP status.">
          ...
          <div className="overflow-x-auto border border-border rounded-xl">
            <table className="w-full text-left text-xs">
              <thead className="border-b border-border bg-muted/40 font-semibold uppercase tracking-wider text-muted-foreground">
                <tr>
                  <th className="p-3">Audit ID</th><th className="p-3">Department</th><th className="p-3">Audit Type</th><th className="p-3">Audit Score</th><th className="p-3">Lead Auditor</th><th className="p-3">Date</th>
                </tr>
              </thead>
