...
export const Route = createFileRoute("/staff/student-dean/discipline")({
  head: () => ({
    meta: [{ title: "Discipline Management — Student Dean" }],
  }),
...
function SubPageComponent() {
  ...
  const headers = ["Case Ref","Student Name","Roll Number","Department","Incident Type","Committee Action","Status"];
  ...
  const handleExportPDF = () => {
    toast.success("Discipline Management PDF Report generated & ready for download.");
    window.print();
  };
  ...
  const handleExportExcel = () => {
    ...
    a.href = url;
    a.download = "discipline-management-ledger.csv";
    a.click();
  ...
  const handleAddSubmit = (e: React.FormEvent) => {
    ...
    setIsAddOpen(false);
    setFormData({});
    toast.success("New Discipline Management record created successfully!");
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Discipline Management</h1>
          <p className="text-sm text-muted-foreground">Campus student discipline records, disciplinary committee hearings, and warnings.</p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Discipline Cases" value="12 Cases" icon={Building2} tone="purple" onClick={() => { setFilter("all"); toast.info("Filter reset to all records"); }} className="cursor-pointer hover:border-primary/50 transition-all" />
        <KpiCard label="Hearings Conducted" value="10 Hearings" icon={Users} tone="success" onClick={() => { setFilter("active"); toast.info("Filtering active records"); }} className="cursor-pointer hover:border-primary/50 transition-all" />
        <KpiCard label="Resolved" value="100%" icon={BookOpen} tone="info" onClick={() => { setFilter("completed"); toast.info("Filtering completed records"); }} className="cursor-pointer hover:border-primary/50 transition-all" />
      ...
      {/* MAIN DATA TABLE */}
      <Panel title="Discipline Management Master Ledger" description="Official executive records, interactive search, filters, pagination, and actions.">
        <div className="space-y-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="relative flex-1 max-w-sm">
              <Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" />
              <Input
                placeholder="Search discipline management records..."
                value={search}
                onChange={(e) => {
      ...
      <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Add New Discipline Management Record</DialogTitle>
      ...
      <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Edit Discipline Management Record</DialogTitle>
      ...
      <Dialog open={isViewOpen} onOpenChange={setIsViewOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Discipline Management Record Details</DialogTitle>
