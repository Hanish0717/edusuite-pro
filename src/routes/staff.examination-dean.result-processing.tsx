...
export const Route = createFileRoute("/staff/examination-dean/result-processing")({
  head: () => ({
    meta: [{ title: "Result Processing — Examination Dean" }],
  }),
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Result Processing</h1>
          <p className="text-sm text-muted-foreground">Automated result processing engine, grace marks rule, and SGPA calculation.</p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Students Processed" value="5,420 Students" icon={Users} tone="purple" />
        <KpiCard label="Pass Rate" value="92.6%" icon={Calendar} tone="success" />
        <KpiCard label="Rule Engine" value="R24 Regulations" icon={Ticket} tone="info" />
        <KpiCard label="Status" value="Processed" icon={Award} tone="warning" />
      </div>

      
      <Panel title="Result Processing Statistics Chart" description="Quantitative examination ledgers across academic departments.">
        <GroupedBarChart
          data={[
      ...
      {/* MAIN DATA TABLE */}
      <Panel title="Result Processing Master Ledger" description="Official Controller of Examinations records, schedules, marks entries, and hall allocations.">
          ...
          <div className="overflow-x-auto border border-border rounded-xl">
            <table className="w-full text-left text-xs">
              <thead className="border-b border-border bg-muted/40 font-semibold uppercase tracking-wider text-muted-foreground">
                <tr>
                  <th className="p-3">Batch Code</th><th className="p-3">Department</th><th className="p-3">Semester</th><th className="p-3">Total Processed</th><th className="p-3">Passed Count</th><th className="p-3">Failed Count</th><th className="p-3">Pass Rate</th><th className="p-3">Status</th>
                </tr>
              </thead>
