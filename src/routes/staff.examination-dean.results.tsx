...
export const Route = createFileRoute("/staff/examination-dean/results")({
  head: () => ({
    meta: [{ title: "Semester Result Publishing — EduSuite Pro" }],
  }),
...
function SubPageComponent() {
  ...
  const rawItems = useMemo(() => {
    return [ { prog: 'B.Tech CSE', sem: 'Sem 6 Autumn', appeared: '480', passRate: '94.2%', status: 'Published' }, { prog: 'B.Tech ECE', sem: 'Sem 6 Autumn', appeared: '360', passRate: '91.8%', status: 'Published' } ];
  }, [data]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Semester Result Publishing</h1>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Overall Pass Rate" value="92.6%" icon={Building2} tone="info" />
        <KpiCard label="Publish SLA" value="14 Days" icon={Users} tone="success" />
        <KpiCard label="Top SGPA" value="9.84 SGPA" icon={ShieldCheck} tone="purple" />
        <KpiCard label="Status" value="Published" icon={CheckCircle2} tone="warning" />
      </div>

      {/* FILTERABLE MAIN TABLE */}
      <Panel title="Semester Result Publishing Ledger" description="Official domain records & ERP status.">
          ...
          <div className="overflow-x-auto border border-border rounded-xl">
            <table className="w-full text-left text-xs">
              <thead className="border-b border-border bg-muted/40 font-semibold uppercase tracking-wider text-muted-foreground">
                <tr>
                  <th className="p-3">Program Name</th><th className="p-3">Semester</th><th className="p-3">Appeared Students</th><th className="p-3">Pass Rate %</th><th className="p-3">Publishing Status</th>
                </tr>
              </thead>
