...
export const Route = createFileRoute("/staff/examination-dean/generate-hall-tickets")({
  head: () => ({
    meta: [{ title: "Generate Hall Tickets — Examination Dean" }],
  }),
...
function SubPageComponent() {
  ...
  const headers = ["Batch Code","Department","Semester","Eligible Students","Generated Count","Generation Date","Status"];
  ...
  const handleExportPDF = () => {
    toast.success("Generate Hall Tickets PDF Report generated & ready for download.");
    window.print();
  };
  ...
  const handleExportExcel = () => {
    ...
    a.href = url;
    a.download = "generate-hall-tickets-ledger.csv";
    a.click();
  ...
  const handleAddSubmit = (e: React.FormEvent) => {
    ...
    setIsAddOpen(false);
    setFormData({});
    toast.success("New Generate Hall Tickets record created successfully!");
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Generate Hall Tickets</h1>
          <p className="text-sm text-muted-foreground">Batch hall ticket generation, fee clearance check, and barcode embedding.</p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Appearing Students" value="5,420 Students" icon={Building2} tone="purple" onClick={() => { setFilter("all"); toast.info("Filter reset to all records"); }} className="cursor-pointer hover:border-primary/50 transition-all" />
        <KpiCard label="Generated" value="4,850 Tickets" icon={Users} tone="success" onClick={() => { setFilter("active"); toast.info("Filtering active records"); }} className="cursor-pointer hover:border-primary/50 transition-all" />
        <KpiCard label="Fee Cleared" value="96.2%" icon={BookOpen} tone="info" onClick={() => { setFilter("completed"); toast.info("Filtering completed records"); }} className="cursor-pointer hover:border-primary/50 transition-all" />
        <KpiCard label="Status" value="Generated" icon={Award} tone="warning" onClick={() => { setFilter("pending"); toast.info("Filtering pending records"); }} className="cursor-pointer hover:border-primary/50 transition-all" />
      </div>

      
      <Panel title="Generate Hall Tickets Analytics Distribution" description="Quantitative ERP breakdown across institutional departments.">
        <GroupedBarChart
          data={[
      ...
      {/* MAIN DATA TABLE */}
      <Panel title="Generate Hall Tickets Master Ledger" description="Official executive records, interactive search, filters, pagination, and actions.">
        <div className="space-y-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="relative flex-1 max-w-sm">
              <Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" />
              <Input
                placeholder="Search generate hall tickets records..."
                value={search}
                onChange={(e) => {
      ...
      <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Add New Generate Hall Tickets Record</DialogTitle>
      ...
      <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Edit Generate Hall Tickets Record</DialogTitle>
      ...
      <Dialog open={isViewOpen} onOpenChange={setIsViewOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Generate Hall Tickets Record Details</DialogTitle>
