...
export const Route = createFileRoute("/staff/academic-dean/academic-performance")({
  head: () => ({
    meta: [{ title: "Academic Performance — Academic Dean" }],
  }),
...
function SubPageComponent() {
  ...
  const headers = ["Department","Appearing Students","Passed Students","Failed Students","Pass %","Average SGPA","Status"];
  ...
  const handleExportPDF = () => {
    toast.success("Academic Performance PDF Report generated & ready for download.");
    window.print();
  };
  ...
  const handleExportExcel = () => {
    ...
    a.href = url;
    a.download = "academic-performance-ledger.csv";
    a.click();
  ...
  const handleAddSubmit = (e: React.FormEvent) => {
    ...
    setIsAddOpen(false);
    setFormData({});
    toast.success("New Academic Performance record created successfully!");
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Academic Performance</h1>
          <p className="text-sm text-muted-foreground">Department-wise SGPA / CGPA performance breakdown, pass percentages, and subject backlogs.</p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Pass Rate" value="92.6% Pass" icon={Building2} tone="purple" onClick={() => { setFilter("all"); toast.info("Filter reset to all records"); }} className="cursor-pointer hover:border-primary/50 transition-all" />
        <KpiCard label="Outstanding (9+)" value="22.4%" icon={Users} tone="success" onClick={() => { setFilter("active"); toast.info("Filtering active records"); }} className="cursor-pointer hover:border-primary/50 transition-all" />
        <KpiCard label="Distinction" value="38.6%" icon={BookOpen} tone="info" onClick={() => { setFilter("completed"); toast.info("Filtering completed records"); }} className="cursor-pointer hover:border-primary/50 transition-all" />
      ...
      {/* MAIN DATA TABLE */}
      <Panel title="Academic Performance Master Ledger" description="Official executive records, interactive search, filters, pagination, and actions.">
        <div className="space-y-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="relative flex-1 max-w-sm">
              <Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" />
              <Input
                placeholder="Search academic performance records..."
                value={search}
                onChange={(e) => {
      ...
      <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Add New Academic Performance Record</DialogTitle>
      ...
      <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Edit Academic Performance Record</DialogTitle>
      ...
      <Dialog open={isViewOpen} onOpenChange={setIsViewOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Academic Performance Record Details</DialogTitle>
