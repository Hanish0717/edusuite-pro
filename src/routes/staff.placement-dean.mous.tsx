...
export const Route = createFileRoute("/staff/placement-dean/mous")({
  head: () => ({
    meta: [{ title: "Corporate MoUs & Partnerships — EduSuite Pro" }],
  }),
...
function SubPageComponent() {
  ...
  const rawItems = useMemo(() => {
    return [ { partner: 'Google Cloud India', scope: 'Cloud Center of Excellence & Placements', date: '2025-06-15', validity: '5 Years', status: 'Active' }, { partner: 'Microsoft R&D', scope: 'AI Lab Support & Campus Hiring', date: '2025-07-20', validity: '5 Years', status: 'Active' } ];
  }, [data]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Corporate MoUs & Partnerships</h1>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Active MoUs" value="42 Corporate MoUs" icon={Building2} tone="info" />
        <KpiCard label="Tier 1 Partners" value="28 Partners" icon={Users} tone="success" />
        <KpiCard label="Validity" value="3-5 Years" icon={ShieldCheck} tone="purple" />
        <KpiCard label="Status" value="Verified" icon={CheckCircle2} tone="warning" />
      </div>

      {/* FILTERABLE MAIN TABLE */}
      <Panel title="Corporate MoUs & Partnerships Ledger" description="Official domain records & ERP status.">
          ...
          <div className="overflow-x-auto border border-border rounded-xl">
            <table className="w-full text-left text-xs">
              <thead className="border-b border-border bg-muted/40 font-semibold uppercase tracking-wider text-muted-foreground">
                <tr>
                  <th className="p-3">Corporate Partner</th><th className="p-3">Agreement Scope</th><th className="p-3">Signing Date</th><th className="p-3">Validity</th><th className="p-3">Status</th>
                </tr>
              </thead>
