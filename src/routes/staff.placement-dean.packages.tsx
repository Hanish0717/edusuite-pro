...
export const Route = createFileRoute("/staff/placement-dean/packages")({
  head: () => ({
    meta: [{ title: "Salary CTC Analytics & Tiers — EduSuite Pro" }],
  }),
...
function SubPageComponent() {
  ...
  const rawItems = useMemo(() => {
    return [ { range: '₹30 LPA - ₹45 LPA', tier: 'Dream Tier 1', count: '42 Offers', share: '5.4%' }, { range: '₹15 LPA - ₹30 LPA', tier: 'Tier 1', count: '185 Offers', share: '23.7%' }, { range: '₹8 LPA - ₹15 LPA', tier: 'Tier 2', count: '420 Offers', share: '53.8%' } ];
  }, [data]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Salary CTC Analytics & Tiers</h1>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Highest Package" value="₹44.5 LPA" icon={Building2} tone="info" />
        <KpiCard label="Average Package" value="₹11.8 LPA" icon={Users} tone="success" />
        <KpiCard label="Median Package" value="₹9.5 LPA" icon={ShieldCheck} tone="purple" />
        <KpiCard label="Status" value="Active" icon={CheckCircle2} tone="warning" />
      </div>

      {/* FILTERABLE MAIN TABLE */}
      <Panel title="Salary CTC Analytics & Tiers Ledger" description="Official domain records & ERP status.">
          ...
          <div className="overflow-x-auto border border-border rounded-xl">
            <table className="w-full text-left text-xs">
              <thead className="border-b border-border bg-muted/40 font-semibold uppercase tracking-wider text-muted-foreground">
                <tr>
                  <th className="p-3">Package CTC Range</th><th className="p-3">Recruiter Tier</th><th className="p-3">Offers Count</th><th className="p-3">Percentage Share</th>
                </tr>
              </thead>
