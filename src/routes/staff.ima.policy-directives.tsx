...
export const Route = createFileRoute("/staff/ima/policy-directives")({
  head: () => ({
    meta: [{ title: "Institutional Policy Directives — EduSuite Pro" }],
  }),
...
function SubPageComponent() {
  ...
  const rawItems = useMemo(() => {
    return data.policyDirectives.map(pd => ({ id: pd.id, title: pd.title, category: pd.category, status: pd.status }));
  }, [data]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Institutional Policy Directives</h1>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Active Policies" value="24 Directives" icon={Building2} tone="info" />
        <KpiCard label="IT Policy" value="Enforced" icon={Users} tone="success" />
        <KpiCard label="Green Campus" value="Net Zero" icon={ShieldCheck} tone="purple" />
        <KpiCard label="Status" value="Active" icon={CheckCircle2} tone="warning" />
      </div>

      {/* FILTERABLE MAIN TABLE */}
      <Panel title="Institutional Policy Directives Ledger" description="Official domain records & ERP status.">
          ...
          <div className="overflow-x-auto border border-border rounded-xl">
            <table className="w-full text-left text-xs">
              <thead className="border-b border-border bg-muted/40 font-semibold uppercase tracking-wider text-muted-foreground">
                <tr>
                  <th className="p-3">Directive ID</th><th className="p-3">Directive Title</th><th className="p-3">Category</th><th className="p-3">Enforcement Status</th>
                </tr>
              </thead>
