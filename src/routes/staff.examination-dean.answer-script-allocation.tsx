...
export const Route = createFileRoute("/staff/examination-dean/answer-script-allocation")({
  head: () => ({
    meta: [{ title: "Answer Script Allocation — Examination Dean" }],
  }),
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Answer Script Allocation</h1>
          <p className="text-sm text-muted-foreground">Digital coding & answer booklet bundle allocation to valuation faculty.</p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Scripts Coded" value="24,800 Scripts" icon={Users} tone="purple" />
        <KpiCard label="Bundles Created" value="248 Bundles" icon={Calendar} tone="success" />
        <KpiCard label="Evaluator Faculty" value="120 Evaluators" icon={Ticket} tone="info" />
        <KpiCard label="Status" value="Active" icon={Award} tone="warning" />
      </div>

      
      <Panel title="Answer Script Allocation Statistics Chart" description="Quantitative examination ledgers across academic departments.">
        <GroupedBarChart
          data={[
      ...
      {/* MAIN DATA TABLE */}
      <Panel title="Answer Script Allocation Master Ledger" description="Official Controller of Examinations records, schedules, marks entries, and hall allocations.">
          ...
          <div className="overflow-x-auto border border-border rounded-xl">
            <table className="w-full text-left text-xs">
              <thead className="border-b border-border bg-muted/40 font-semibold uppercase tracking-wider text-muted-foreground">
                <tr>
                  <th className="p-3">Bundle ID</th><th className="p-3">Subject Code</th><th className="p-3">Script Count</th><th className="p-3">Assigned Evaluator</th><th className="p-3">Valuation Center</th><th className="p-3">Status</th>
                </tr>
              </thead>
