...
export const Route = createFileRoute("/staff/finance-dean/department-budgets")({
  head: () => ({
    meta: [{ title: "Department Budget Allocations — EduSuite Pro" }],
  }),
...
function SubPageComponent() {
  ...
  const rawItems = useMemo(() => {
    return data.deptBudgets.map(b => ({ dept: b.dept, allocated: b.allocated, spent: b.spent, percentage: b.percentage + '%' }));
  }, [data]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Department Budget Allocations</h1>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Total Budget" value="₹48.5 Cr" icon={Building2} tone="info" />
        <KpiCard label="Spent Amount" value="₹36.2 Cr" icon={Users} tone="success" />
        <KpiCard label="Avg Utilization" value="86.4%" icon={ShieldCheck} tone="purple" />
        <KpiCard label="Status" value="Active" icon={CheckCircle2} tone="warning" />
      </div>

      {/* FILTERABLE MAIN TABLE */}
      <Panel title="Department Budget Allocations Ledger" description="Official domain records & ERP status.">
          ...
          <div className="overflow-x-auto border border-border rounded-xl">
            <table className="w-full text-left text-xs">
              <thead className="border-b border-border bg-muted/40 font-semibold uppercase tracking-wider text-muted-foreground">
                <tr>
                  <th className="p-3">Department</th><th className="p-3">Allocated Budget</th><th className="p-3">Spent Amount</th><th className="p-3">Utilization %</th>
                </tr>
              </thead>
