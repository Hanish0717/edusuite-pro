...
export const Route = createFileRoute("/staff/finance-dean/purchase-requests")({
  head: () => ({
    meta: [{ title: "Purchase Requests — Finance Dean" }],
  }),
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Requisition requests for campus hardware, furniture, and IT equipment.</p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Requisitions" value="12 Requests" icon={Wallet} tone="purple" />
        <KpiCard label="Est Total" value="₹45.0 Lacs" icon={CreditCard} tone="success" />
        <KpiCard label="Budget Clearance" value="Verified" icon={TrendingUp} tone="info" />
        <KpiCard label="Status" value="Approved" icon={ShieldCheck} tone="warning" />
      </div>

      
      <Panel title="Purchase Requests Distribution Chart" description="Quantitative financial ledger across academic departments.">
        <GroupedBarChart
          data={[
      ...
      {/* MAIN DATA TABLE */}
      <Panel title="Purchase Requests Master Ledger" description="Official institutional financial ledgers, audit vouchers, and budget allocations.">
          ...
          <div className="overflow-x-auto border border-border rounded-xl">
            <table className="w-full text-left text-xs">
              <thead className="border-b border-border bg-muted/40 font-semibold uppercase tracking-wider text-muted-foreground">
                <tr>
                  <th className="p-3">Req Ref</th><th className="p-3">Department</th><th className="p-3">Item Requested</th><th className="p-3">Quantity</th><th className="p-3">Estimated Cost</th><th className="p-3">Financial Approval</th><th className="p-3">Status</th>
                </tr>
              </thead>
