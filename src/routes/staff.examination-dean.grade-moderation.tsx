...
import { getExaminationDeanDashboardData } from "@/lib/deansService";

export const Route = createFileRoute("/staff/examination-dean/grade-moderation")({
  head: () => ({
    meta: [{ title: "Grade Moderation Committee — EduSuite Pro" }],
  }),
  component: SubPageComponent,
});

function SubPageComponent() {
  const data = useMemo(() => getExaminationDeanDashboardData(), []);
  ...
  const rawItems = useMemo(() => {
    return [ { code: 'CS401', evaluator: 'Dr. S. K. Gupta', avg: '76.4%', delta: '+2.0 Marks (Standardized)', status: 'Approved' }, { code: 'EC304', evaluator: 'Dr. Meera Rao', avg: '74.2%', delta: '+1.5 Marks (Standardized)', status: 'Approved' } ];
  }, [data]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Grade Moderation Committee</h1>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Evaluated Papers" value="4,850 Papers" icon={Building2} tone="info" />
        <KpiCard label="Pass Percentage" value="92.6%" icon={Users} tone="success" />
        <KpiCard label="Moderation Status" value="Complete" icon={ShieldCheck} tone="purple" />
        <KpiCard label="Status" value="Verified" icon={CheckCircle2} tone="warning" />
      </div>

      {/* FILTERABLE MAIN TABLE */}
      <Panel title="Grade Moderation Committee Ledger" description="Official domain records & ERP status.">
          ...
          <div className="overflow-x-auto border border-border rounded-xl">
            <table className="w-full text-left text-xs">
              <thead className="border-b border-border bg-muted/40 font-semibold uppercase tracking-wider text-muted-foreground">
                <tr>
                  <th className="p-3">Course Code</th><th className="p-3">Faculty Evaluator</th><th className="p-3">Class Average</th><th className="p-3">Grade Moderation Delta</th><th className="p-3">Review Status</th>
                </tr>
              </thead>
