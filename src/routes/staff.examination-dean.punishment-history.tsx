...
export const Route = createFileRoute("/staff/examination-dean/punishment-history")({
  head: () => ({
    meta: [{ title: "Punishment History — Examination Dean" }],
  }),
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Punishment History</h1>
          <p className="text-sm text-muted-foreground">Past malpractice disciplinary actions, exam cancellations, and fine collection.</p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Disciplinary Actions" value="8 Cases Total" icon={Users} tone="purple" />
        <KpiCard label="Fines Collected" value="100%" icon={Calendar} tone="success" />
        <KpiCard label="Enforcement Audit" value="Passed" icon={Ticket} tone="info" />
        <KpiCard label="Status" value="Archived" icon={Award} tone="warning" />
      </div>

      

      {/* MAIN DATA TABLE */}
      <Panel title="Punishment History Master Ledger" description="Official Controller of Examinations records, schedules, marks entries, and hall allocations.">
          ...
          <div className="overflow-x-auto border border-border rounded-xl">
            <table className="w-full text-left text-xs">
              <thead className="border-b border-border bg-muted/40 font-semibold uppercase tracking-wider text-muted-foreground">
                <tr>
                  <th className="p-3">Case Ref</th><th className="p-3">Student Name</th><th className="p-3">Academic Session</th><th className="p-3">Punishment Enforced</th><th className="p-3">Fine Paid</th><th className="p-3">Status</th>
                </tr>
              </thead>
