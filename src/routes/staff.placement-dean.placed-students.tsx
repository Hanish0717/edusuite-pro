...
import { getPlacementDeanDashboardData } from "@/lib/deansService";

export const Route = createFileRoute("/staff/placement-dean/placed-students")({
  head: () => ({
    meta: [{ title: "Placed Students Master Ledger — EduSuite Pro" }],
  }),
  component: SubPageComponent,
});

function SubPageComponent() {
  const data = useMemo(() => getPlacementDeanDashboardData(), []);
  ...
  const rawItems = useMemo(() => {
    return [ { name: 'K. Sai Teja', roll: '22CS101', dept: 'CSE', company: 'Google Cloud India', ctc: '₹44.5 LPA', status: 'Offer Dispatched' }, { name: 'Ananya Roy', roll: '22EC104', dept: 'ECE', company: 'Microsoft R&D', ctc: '₹38.0 LPA', status: 'Offer Dispatched' } ];
  }, [data]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Placed Students Master Ledger</h1>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Placement Rate" value="92.6%" icon={Building2} tone="info" />
        <KpiCard label="Total Students Placed" value="780 Students" icon={Users} tone="success" />
        <KpiCard label="Avg Package CTC" value="₹11.8 LPA" icon={ShieldCheck} tone="purple" />
        <KpiCard label="Status" value="Verified" icon={CheckCircle2} tone="warning" />
      </div>

      {/* FILTERABLE MAIN TABLE */}
      <Panel title="Placed Students Master Ledger Ledger" description="Official domain records & ERP status.">
          ...
          <div className="overflow-x-auto border border-border rounded-xl">
            <table className="w-full text-left text-xs">
              <thead className="border-b border-border bg-muted/40 font-semibold uppercase tracking-wider text-muted-foreground">
                <tr>
                  <th className="p-3">Student Name</th><th className="p-3">Roll Number</th><th className="p-3">Department</th><th className="p-3">Recruiter Company</th><th className="p-3">Package CTC</th><th className="p-3">Status</th>
                </tr>
              </thead>
