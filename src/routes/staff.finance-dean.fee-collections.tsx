...
export const Route = createFileRoute("/staff/finance-dean/fee-collections")({
  head: () => ({
    meta: [{ title: "Fee Collections Ledger — EduSuite Pro" }],
  }),
...
function SubPageComponent() {
  ...
  const rawItems = useMemo(() => {
    return data.feeCollections.map(fc => ({ category: fc.category, expected: fc.expected, collected: fc.collected, pending: fc.pending }));
  }, [data]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Fee Collections Ledger</h1>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Expected Fees" value="₹32.7 Cr" icon={Building2} tone="info" />
        <KpiCard label="Collected Fees" value="₹31.3 Cr" icon={Users} tone="success" />
        <KpiCard label="Pending Dues" value="₹1.4 Cr" icon={ShieldCheck} tone="purple" />
        <KpiCard label="Status" value="Active" icon={CheckCircle2} tone="warning" />
      </div>

      {/* FILTERABLE MAIN TABLE */}
      <Panel title="Fee Collections Ledger Ledger" description="Official domain records & ERP status.">
          ...
          <div className="overflow-x-auto border border-border rounded-xl">
            <table className="w-full text-left text-xs">
              <thead className="border-b border-border bg-muted/40 font-semibold uppercase tracking-wider text-muted-foreground">
                <tr>
                  <th className="p-3">Category Name</th><th className="p-3">Expected Revenue</th><th className="p-3">Collected Amount</th><th className="p-3">Pending Dues</th>
                </tr>
              </thead>
