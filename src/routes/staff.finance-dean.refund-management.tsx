...
export const Route = createFileRoute("/staff/finance-dean/refund-management")({
  head: () => ({
    meta: [{ title: "Refund Management — Finance Dean" }],
  }),
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Refund Management</h1>
          <p className="text-sm text-muted-foreground">Caution deposit refunds, seat cancellation fee refunds, and bank payouts.</p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Refund Claims" value="18 Requests" icon={Wallet} tone="purple" />
        <KpiCard label="Total Refunded" value="₹8.50 Lacs" icon={CreditCard} tone="success" />
        <KpiCard label="SLA Clearance" value="100%" icon={TrendingUp} tone="info" />
        <KpiCard label="Status" value="Active" icon={ShieldCheck} tone="warning" />
      </div>

      

      {/* MAIN DATA TABLE */}
      <Panel title="Refund Management Master Ledger" description="Official institutional financial ledgers, audit vouchers, and budget allocations.">
          ...
          <div className="overflow-x-auto border border-border rounded-xl">
            <table className="w-full text-left text-xs">
              <thead className="border-b border-border bg-muted/40 font-semibold uppercase tracking-wider text-muted-foreground">
                <tr>
                  <th className="p-3">Refund Ref</th><th className="p-3">Student Name</th><th className="p-3">Roll Number</th><th className="p-3">Refund Reason</th><th className="p-3">Refund Amount</th><th className="p-3">Bank Details</th><th className="p-3">Status</th>
                </tr>
              </thead>
