...
export const Route = createFileRoute("/staff/finance-dean/audits")({
  head: () => ({
    meta: [{ title: "Statutory Financial Audits — EduSuite Pro" }],
  }),
...
function SubPageComponent() {
  ...
  const rawItems = useMemo(() => {
    return [ { year: 'FY 2025-26', auditor: 'Deloitte Haskins & Sells LLP', scope: 'Full Statutory Balance Sheet', opinion: 'Clean Unqualified Pass', status: 'Verified' } ];
  }, [data]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Statutory Financial Audits</h1>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Audit Opinion" value="Unqualified (Clean)" icon={Building2} tone="info" />
        <KpiCard label="Financial Year" value="FY 2025-26" icon={Users} tone="success" />
        <KpiCard label="Tax Compliance" value="100%" icon={ShieldCheck} tone="purple" />
        <KpiCard label="Status" value="Verified" icon={CheckCircle2} tone="warning" />
      </div>

      {/* FILTERABLE MAIN TABLE */}
      <Panel title="Statutory Financial Audits Ledger" description="Official domain records & ERP status.">
          ...
          <div className="overflow-x-auto border border-border rounded-xl">
            <table className="w-full text-left text-xs">
              <thead className="border-b border-border bg-muted/40 font-semibold uppercase tracking-wider text-muted-foreground">
                <tr>
                  <th className="p-3">Audit Year</th><th className="p-3">External Auditor</th><th className="p-3">Audit Scope</th><th className="p-3">Audit Opinion</th><th className="p-3">Status</th>
                </tr>
              </thead>
