...
export const Route = createFileRoute("/staff/academic-dean/faculty-performance")({
  head: () => ({
    meta: [{ title: "Faculty Performance — Academic Dean" }],
  }),
...
function SubPageComponent() {
  ...
  const headers = ["Faculty Name","Department","Feedback Rating","Pass % Delivered","API Score","Status"];
  ...
  const handleExportPDF = () => {
    toast.success("Faculty Performance PDF Report generated & ready for download.");
    window.print();
  };
  ...
  const handleExportExcel = () => {
    ...
    a.href = url;
    a.download = "faculty-performance-ledger.csv";
    a.click();
  ...
  const handleAddSubmit = (e: React.FormEvent) => {
    ...
    setIsAddOpen(false);
    setFormData({});
    toast.success("New Faculty Performance record created successfully!");
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Faculty Performance</h1>
          <p className="text-sm text-muted-foreground">Annual academic performance index (API), student feedback ratings, and publication credits.</p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Avg Feedback" value="4.7 / 5.0" icon={Building2} tone="purple" onClick={() => { setFilter("all"); toast.info("Filter reset to all records"); }} className="cursor-pointer hover:border-primary/50 transition-all" />
        <KpiCard label="API Clearance" value="100%" icon={Users} tone="success" onClick={() => { setFilter("active"); toast.info("Filtering active records"); }} className="cursor-pointer hover:border-primary/50 transition-all" />
        <KpiCard label="Pass Rate Avg" value="94.8%" icon={BookOpen} tone="info" onClick={() => { setFilter("completed"); toast.info("Filtering completed records"); }} className="cursor-pointer hover:border-primary/50 transition-all" />
      ...
      {/* MAIN DATA TABLE */}
      <Panel title="Faculty Performance Master Ledger" description="Official executive records, interactive search, filters, pagination, and actions.">
        <div className="space-y-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="relative flex-1 max-w-sm">
              <Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" />
              <Input
                placeholder="Search faculty performance records..."
                value={search}
                onChange={(e) => {
      ...
      <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Add New Faculty Performance Record</DialogTitle>
      ...
      <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Edit Faculty Performance Record</DialogTitle>
      ...
      <Dialog open={isViewOpen} onOpenChange={setIsViewOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Faculty Performance Record Details</DialogTitle>
