...
export const Route = createFileRoute("/staff/placement-dean/internships")({
  head: () => ({
    meta: [{ title: "Corporate Internship Drives — EduSuite Pro" }],
  }),
...
function SubPageComponent() {
  ...
  const rawItems = useMemo(() => {
    return [ { company: 'Google India', role: 'Software Engineering Intern', stipend: '₹1,20,000 / Mo', count: '14 Interns', status: 'Selected' }, { company: 'Qualcomm India', role: 'VLSI Engineering Intern', stipend: '₹85,000 / Mo', count: '22 Interns', status: 'Selected' } ];
  }, [data]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Corporate Internship Drives</h1>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Internships Offered" value="340 Offers" icon={Building2} tone="info" />
        <KpiCard label="Highest Stipend" value="₹1.2 Lakh / Mo" icon={Users} tone="success" />
        <KpiCard label="Avg Stipend" value="₹45,000 / Mo" icon={ShieldCheck} tone="purple" />
        <KpiCard label="Status" value="Active" icon={CheckCircle2} tone="warning" />
      </div>

      {/* FILTERABLE MAIN TABLE */}
      <Panel title="Corporate Internship Drives Ledger" description="Official domain records & ERP status.">
          ...
          <div className="overflow-x-auto border border-border rounded-xl">
            <table className="w-full text-left text-xs">
              <thead className="border-b border-border bg-muted/40 font-semibold uppercase tracking-wider text-muted-foreground">
                <tr>
                  <th className="p-3">Company Name</th><th className="p-3">Internship Role</th><th className="p-3">Monthly Stipend</th><th className="p-3">Selected Students</th><th className="p-3">Status</th>
                </tr>
              </thead>
