...
export const Route = createFileRoute("/staff/examination-dean/exam-schedules")({
  head: () => ({
    meta: [{ title: "End-Semester Examination Schedules — EduSuite Pro" }],
  }),
...
function SubPageComponent() {
  ...
  const rawItems = useMemo(() => {
    return data.examSchedules.map(s => ({ code: s.code, subject: s.subject, date: s.date, session: s.session, students: s.students }));
  }, [data]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-foreground">End-Semester Examination Schedules</h1>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Scheduled Exams" value="184 Papers" icon={Building2} tone="info" />
        <KpiCard label="Registered Students" value="4,850" icon={Users} tone="success" />
        <KpiCard label="Exam Halls" value="Halls 1-12" icon={ShieldCheck} tone="purple" />
        <KpiCard label="Status" value="Active" icon={CheckCircle2} tone="warning" />
      </div>

      {/* FILTERABLE MAIN TABLE */}
      <Panel title="End-Semester Examination Schedules Ledger" description="Official domain records & ERP status.">
          ...
          <div className="overflow-x-auto border border-border rounded-xl">
            <table className="w-full text-left text-xs">
              <thead className="border-b border-border bg-muted/40 font-semibold uppercase tracking-wider text-muted-foreground">
                <tr>
                  <th className="p-3">Course Code</th><th className="p-3">Subject Name</th><th className="p-3">Exam Date</th><th className="p-3">Session Slot</th><th className="p-3">Enrolled Students</th>
                </tr>
              </thead>
