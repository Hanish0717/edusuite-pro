...
export const Route = createFileRoute("/staff/placement-dean/reports")({
  head: () => ({
    meta: [{ title: "Placement Statistics & Reports — EduSuite Pro" }],
  }),
...
function SubPageComponent() {
  ...
  const rawItems = useMemo(() => {
    return data.reports.map(r => ({ title: r.title, metric: r.metric, date: r.date, status: 'Verified' }));
  }, [data]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Placement Statistics & Reports</h1>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Total Reports" value="15 Statistics Reports" icon={Building2} tone="info" />
        <KpiCard label="Placement Rate" value="92.6%" icon={Users} tone="success" />
        <KpiCard label="Recruiter Rating" value="4.8 / 5" icon={ShieldCheck} tone="purple" />
        <KpiCard label="Status" value="Active" icon={CheckCircle2} tone="warning" />
      </div>

      {/* FILTERABLE MAIN TABLE */}
      <Panel title="Placement Statistics & Reports Ledger" description="Official domain records & ERP status.">
          ...
          <div className="overflow-x-auto border border-border rounded-xl">
            <table className="w-full text-left text-xs">
              <thead className="border-b border-border bg-muted/40 font-semibold uppercase tracking-wider text-muted-foreground">
                <tr>
                  <th className="p-3">Report Title</th><th className="p-3">Metric Highlight</th><th className="p-3">Generated Date</th><th className="p-3">Status</th>
                </tr>
              </thead>
