...
export const Route = createFileRoute("/staff/examination-dean/confidential-storage")({
  head: () => ({
    meta: [{ title: "Confidential Storage — Examination Dean" }],
  }),
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Confidential Storage</h1>
          <p className="text-sm text-muted-foreground">Secure encrypted vault storage for question paper master files.</p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Vault Security" value="2FA Biometric Lock" icon={Users} tone="purple" />
        <KpiCard label="Master Files" value="184 Files" icon={Calendar} tone="success" />
        <KpiCard label="Audit Clearance" value="Secure" icon={Ticket} tone="info" />
        <KpiCard label="Status" value="Secure" icon={Award} tone="warning" />
      </div>

      

      {/* MAIN DATA TABLE */}
      <Panel title="Confidential Storage Master Ledger" description="Official Controller of Examinations records, schedules, marks entries, and hall allocations.">
          ...
          <div className="overflow-x-auto border border-border rounded-xl">
            <table className="w-full text-left text-xs">
              <thead className="border-b border-border bg-muted/40 font-semibold uppercase tracking-wider text-muted-foreground">
                <tr>
                  <th className="p-3">Paper Code</th><th className="p-3">Subject Name</th><th className="p-3">Vault Storage ID</th><th className="p-3">Decryption Key Status</th><th className="p-3">Access Control</th><th className="p-3">Status</th>
                </tr>
              </thead>
