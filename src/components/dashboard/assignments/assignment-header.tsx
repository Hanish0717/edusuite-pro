import { ClipboardList, PlusCircle, RefreshCw, FileSpreadsheet } from "lucide-react";
...
export function AssignmentHeader({ academicYear, semester, onRefresh, onCreateTrigger }: AssignmentHeaderProps) {
  const handleExport = () => {
  ...
  return (
    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-border pb-4 text-xs">
      <div>
        <h1 className="font-display text-2xl font-extrabold tracking-tight">Assignment Management</h1>
        <p className="text-xs text-muted-foreground flex items-center gap-1.5 mt-1 font-medium">
          <ClipboardList className="size-3.5" /> Academic Year {academicYear} &middot; Semester {semester}
        </p>
      </div>
      ...
      <div className="flex items-center gap-2.5 shrink-0">
        ...
        >
          <PlusCircle className="size-3.5 mr-2" /> Create Assignment
        </Button>
      </div>
    </div>
  );
