...
export function StudentTimetableCard({ timetable, onViewFullTimetable }: StudentTimetableProps) {
  return (
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-500 font-medium">
              4 Lectures Conducted in Academic Block A & B
            </p>
          </div>
        </div>
          ...
          return (
            <div
              ...
              className={`p-4 rounded-xl border transition-all flex flex-col md:flex-row md:items-center justify-between gap-3 ${
                isCurrent
                  ...
                  : isCompleted
                  ? "border-slate-200/80 dark:border-slate-800/80 bg-slate-50/50 dark:bg-slate-800/30 opacity-80"
            ...
            >
              ...
              <div className="flex items-center gap-3 min-w-[170px]">
                ...
                >
                  {item.status === "Current" && "● LIVE NOW"}
                  {item.status === "Next" && "NEXT CLASS"}
              ...
              {/* RIGHT: JOIN ONLINE BUTTON IF APPLICABLE */}
              {item.isOnline && item.joinUrl && (
                ...
                >
                  <Video className="h-3.5 w-3.5" /> Join Virtual Class
                </a>
              )}
            </div>
          );
