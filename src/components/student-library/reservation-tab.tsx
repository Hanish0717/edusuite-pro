    ...
    <div className="space-y-4">
      ...
      {reservations.length > 0 ? (
            ...
            >
              <div className="space-y-2">
                ...
                <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-800/40 border border-slate-100 dark:border-slate-800 grid grid-cols-2 gap-2 text-[11px] font-mono">
                  <div>
                    <span className="text-slate-400 text-[10px] block uppercase">Reserved On</span>
                    <span className="text-slate-800 dark:text-slate-200 font-bold">{res.reservedDate}</span>
                  </div>
                  <div>
                    <span className="text-slate-400 text-[10px] block uppercase">Est. Availability</span>
                ...
                {res.status === "Ready for Pickup" ? (
                  ...
                  <Button
                    onClick={() => {
                      ...
                      toast.info(`Cancelled hold queue for "${res.title}".`);
                    }}
                    variant="outline"
        ...
        <div className="p-12 text-center rounded-2xl border border-dashed border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 space-y-2">
          <BookmarkCheck className="h-10 w-10 text-slate-400 mx-auto" />
          <h4 className="font-bold text-xs text-slate-700 dark:text-slate-300">
            No active book holds or reservations
          </h4>
          <p className="text-xs text-slate-500 font-medium">
            Books placed on hold will appear here with queue status and pickup alerts.
          </p>
        </div>
      )}
    </div>
