...
export function TransportModuleView() {
  ...
  const totalPassHolders     = routes.reduce((s, r) => s + (r.passHoldersCount || 0), 0);
  ...
  const passRevenue   = passes.reduce((s, p) => s + (p.annualFee || 0), 0);

  // ── Fleet route table rows ────────────────────────────────────────────────
  const fleetRows = routes.map((r, idx) => {
    const hd = FLEET_HEALTH_DATA[idx] || FLEET_HEALTH_DATA[0];
    const occ = r.capacity > 0 ? (((r.passHoldersCount || 0) / r.capacity) * 100).toFixed(0) : "0";
                ...
                <tbody className="divide-y divide-border/60">
                  {filteredPasses.map(p => (
                    <tr key={p.id} className="hover:bg-muted/20 transition-colors">
                      ...
                      <td className="py-3 px-3 font-mono font-bold text-foreground">₹{(p.annualFee || 0).toLocaleString()}</td>
                      <td className="py-3 px-3">
